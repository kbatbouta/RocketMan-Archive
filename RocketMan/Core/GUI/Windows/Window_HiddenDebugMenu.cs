...
namespace RocketMan
{
    public class Window_HiddenDebugMenu : Window
    {
        ...
        private void DoSingleTickIncrementsControls(Rect inRect)
        {
            ...
            Widgets.CheckboxLabeled(inRect.TopHalf(), "Do singleTick increments controls", ref RocketDebugPrefs.singleTickIncrement);
            if (Widgets.ButtonText(inRect.BottomHalf(), "Do single tick"))
            {
                if (!RocketDebugPrefs.singleTickIncrement)
                {
                    RocketDebugPrefs.singleTickIncrement = true;
                }

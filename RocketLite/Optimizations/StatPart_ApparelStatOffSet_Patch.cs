...
namespace RocketLite.Optimizations
{
    public static class StatPart_ApparelStatOffSet_Skipper_Patch
    {
        ...
        {
            return AccessTools.Method(typeof(StatPart_ApparelStatOffset), nameof(StatPart_ApparelStatOffset.TransformValue));
        }
        ...
        public static bool Prefix(StatPart_ApparelStatOffset __instance, StatRequest req,
            ref float val)
        {
            if (req.HasThing && req.thingInt is Pawn pawn)
            {
        ...
        public static void Postfix(StatPart_ApparelStatOffset __instance, StatRequest req,
            ref float val)
        {
            if (!skip) cache[curKey] = new Pair<float, int>(Mathf.Abs(val - curValue), (req.thingInt as Pawn).GetSignature());
        }
